...
| Data Model | Seed hints through migrations | `hints` already exist in schema, but no real hint data has been seeded yet for MVP objectives. | Create a later EVO for first real hint seeds once route-detail payloads need them. |
| Data Model | Seed unlockable content through migrations | `unlockable_contents` exist in schema, but the MVP still lacks real unlockable narrative content in D1. | Add a later EVO once the route detail / objective completion flow is ready to expose unlockable content. |
